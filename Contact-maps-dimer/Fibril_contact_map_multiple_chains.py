# Modified version focusing on quadrant contact map visualization with enhanced text output
...
class FibrilConfig:
    """Configuration for fibril contact analysis with inter-protein analysis"""
    
    def __init__(self):
        self.cutoff = 5.0
        self.pdb_filename = "6WQK.pdb"
        self.auto_detect_range = True
        self.manual_start_residue = 1
        self.manual_end_residue = 100
        self.dpi = 300
        self.figsize_heatmap = (12, 10)
        self.figsize_barplot = (12, 6)
        self.tick_interval = 10
        self.exclude_hydrogens = True
        self.chain_analysis = True
        self.contact_persistence = True
        # New inter-protein specific settings
        self.inter_protein_analysis = True
        self.separate_inter_intra = True
        self.min_inter_protein_distance = 0.0  # Minimum distance for inter-protein contacts
...
class FibrilPDBProcessor:
    """Process PDB files for fibril contact analysis with robust parsing"""
    ...
    def robust_pdb_parser(self, filename: str) -> Tuple[np.ndarray, Dict, Dict]:
        ...
        try:
            ...
            # Build residue and chain info - store residue names properly
            self.residue_info = {}
            for atom in atoms:
    ...
    def analyze_structure_with_chain_separation(self, filename: str) -> Tuple[np.ndarray, Dict]:
        ...
        for chain in chain_list:
            chain_residues = sorted(self.chain_info[chain])  # ints
            start = current_pos
        ...
        for i, j in zip(ci, cj):
            if i >= j:  # skip self/duplicate
                continue
            key_i = tuple(residue_keys[i])    # (chain, resid)
            key_j = tuple(residue_keys[j])

            if key_i == key_j:
                continue  # same residue (same chain & resid), skip
        ...
        analysis_info = {
            ...
            'global_residue_order': global_residue_order,  # list of (chain, resid)
            'matrix_size': matrix_size,
            'chain_info': self.chain_info,                 # {chain: [resids]}
            'residue_info': self.residue_info,
            'contacts_list': contacts_list  # Add contacts list to analysis info
        }
        
        # Generate enhanced text output
        self._write_contact_interactions(contacts_list, filename)
        
        print(f"\nTotal contacts found: {len(contacts_list)}")
        for c in contacts_list[:10]:  # show first 10 for preview
    ...
    def _write_contact_interactions(self, contacts_list: List[Tuple], pdb_filename: str):
        ...
        # 2. Simple format file (just amino acid + number)
        simple_file = f"{base_name}_simple_interactions_{cutoff_str}.txt"
        with open(simple_file, "w") as f:
            f.write("SIMPLE CONTACT LIST\n")
            f.write("=" * 30 + "\n")
            f.write(f"Format: AA1-Number ↔ AA2-Number\n\n")
            ...
            for contact in sorted_contacts:
                ...
                res1_name = self.residue_info.get(key1, 'UNK')
                res2_name = self.residue_info.get(key2, 'UNK')
                f.write(f"{res1_name}{key1[1]}-{res2_name}{key2[1]}\n")
        
        # 3. CSV format for data analysis
        csv_file = f"{base_name}_interactions_{cutoff_str}.csv"
        with open(csv_file, "w") as f:
            f.write("Residue1,Number1,Chain1,Residue2,Number2,Chain2,ContactType\n")
            ...
            for contact in sorted_contacts:
                ...
                contact_type = "INTRA" if chain1 == chain2 else "INTER"
                
                f.write(f"{res1_name},{res1},{chain1},{res2_name},{res2},{chain2},{contact_type}\n")
        
        self.logger.info(f"Contact interactions written to:")
        self.logger.info(f"  - Detailed: {detailed_file}")
        self.logger.info(f"  - Simple: {simple_file}")
        self.logger.info(f"  - CSV: {csv_file}")
    
    def _calculate_contacts_efficient(self, positions, residue_ids, chain_ids, residue_to_global_idx):
        """Memory-efficient contact calculation for large structures"""
        matrix_size = len(residue_to_global_idx)
        contact_matrix = np.zeros((matrix_size, matrix_size), dtype=np.int32)
        ...
        for i in range(0, n_atoms, chunk_size):
            ...
            for j in range(i, n_atoms, chunk_size):
                end_j = min(j + chunk_size, n_atoms)
                chunk_j = positions[j:end_j]
                
                # Calculate distances for this chunk
                dist_chunk = np.linalg.norm(chunk_i[:, np.newaxis] - chunk_j[np.newaxis, :], axis=2)
                contacts = dist_chunk < self.config.cutoff
                
                # Process contacts in this chunk
                contact_indices = np.where(contacts)
                for local_i, local_j in zip(contact_indices[0], contact_indices[1]):
                    global_atom_i = i + local_i
                    global_atom_j = j + local_j
                    
                    if global_atom_i >= global_atom_j:
                        continue
                    
                    res_i = residue_ids[global_atom_i]
                    res_j = residue_ids[global_atom_j]
                    
                    if res_i != res_j and res_i in residue_to_global_idx and res_j in residue_to_global_idx:
                        matrix_i = residue_to_global_idx[res_i]
                        matrix_j = residue_to_global_idx[res_j]
                        contact_matrix[matrix_i, matrix_j] = 1
...
class QuadrantContactVisualizer:
        ...
        try:
            with timer("Quadrant contact map visualization", self.logger):
                ...
                if len(chains) < 2:
                    ...
                    # Fallback to regular contact map
                ...
                colors_intra = plt.cm.Blues(np.linspace(0, 1, 128))
                colors_inter = plt.cm.Reds(np.linspace(0, 1, 128))
                ...
                # Add chain boundary lines (dashed)
                for chain in chains:
                    start, end = chain_boundaries[chain]
                    if start > 0:
                        ax.axhline(y=start-0.5, color='black', linewidth=2, linestyle='--', alpha=0.8)
                        ax.axvline(x=start-0.5, color='black', linewidth=2, linestyle='--', alpha=0.8)

                # Add bold frame around the entire plot
    ...
    def _find_chain_for_residue(self, residue_num: int, analysis_info: Dict) -> str:
        """Find which chain a residue belongs to"""
        chain_info = analysis_info['chain_info']
        for chain_id, residues in chain_info.items():
            if residue_num in residues:
                return chain_id
        return 'UNK'
    
    def _configure_ticks_and_labels(self, ax, analysis_info: Dict):
        """Configure axis ticks and labels with tick every 10 residues"""
        chains = analysis_info['chains']
        chain_boundaries = analysis_info['chain_boundaries']
        global_residue_order = analysis_info['global_residue_order']
        matrix_size = analysis_info['matrix_size']
        ...
        # Generate ticks every 10 residues across all chains
        for i in range(0, matrix_size, 10):
            if i < matrix_size:
                tick_positions.append(i)
                tick_labels.append(f"{global_residue_order[i][1]}")
        
        # Always add the last position
        if matrix_size - 1 not in tick_positions:
            tick_positions.append(matrix_size - 1)
            tick_labels.append(f"{global_residue_order[matrix_size - 1][1]}")
    ...
    def _plot_single_chain_fallback(self, contact_matrix: np.ndarray, analysis_info: Dict) -> bool:
        ...
        try:
            ...
            # Simple contact map for single chain
...
class QuadrantContactAnalyzer:
    ...
    def print_summary(self, contact_matrix: np.ndarray, analysis_info: Dict):
        ...
        print(f"  - Detailed interactions: {base_name}_interactions_{cutoff_str}.txt")
        print(f"  - Simple interactions: {base_name}_simple_interactions_{cutoff_str}.txt")
        print(f"  - CSV format: {base_name}_interactions_{cutoff_str}.csv")
        print("="*60)
...
def analyze_quadrant_contacts(pdb_file: str = "6WQK.pdb", cutoff: float = 5.0) -> bool:
    ...
    print(f"Running Enhanced Quadrant Contact Analysis")
...
if __name__ == "__main__":
    try:
        ...
        if success:
            ...
            print("  - TXT: Detailed and simple interaction lists")
            print("  - CSV: Machine-readable format for further analysis")
        else:
