...
two files per ion type per pH condition:

    1. A **legacy text file** (whitespace-delimited, one atom-level
       contact per line) consumed by the downstream contact analysis
       pipeline::

           frame <n> <resname> <resid> chain <segid> interacts_with_<ion>_<ionresid>

    2. A **CSV file** suitable for repository deposition, containing the
       same data with explicit column headers::
...
Pipeline context
----------------
This is **Step 1** of the analysis pipeline.  Its text-file outputs are
read by ``contact_analysis_pipeline.py`` (Step 2) and
``contact_analysis_fragment.py`` (Step 2b).

Usage
-----
    python Extract_protein_ion_interactions.py

Adjust the constants in the ``Configuration`` section below.
...
def export_interactions_csv(filepath: Path, records: List[Dict]) -> None:
    ...
    The CSV contains one row per atom-level contact with explicit column
    headers, making it self-describing for anyone downloading the raw data.
...
def run_single_condition(fold: str, ph: str, cutoff: float) -> None:
    ...
    ----------
    fold : str
        Fold identifier (e.g. ``"1-2"``).
    ph : str
        pH label used in filenames (e.g. ``"74"`` for pH 7.4).
    cutoff : float
    ...
    print(f"\n{'=' * 60}")
    print(f"  Fold {fold}  |  pH {ph}  |  cutoff {cutoff:.1f} Å")
...
def main() -> None:
    ...
    print(
        f"Protein–ion interaction extraction  |  "
