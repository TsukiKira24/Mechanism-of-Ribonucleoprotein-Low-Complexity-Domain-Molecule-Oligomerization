# -*- coding: utf-8 -*-
"""
Violin Plot Analysis of Fibril Contacts in Dimer Simulations
Analyzes how well internal contacts within each protomer (A and B) 
...
ph_colors = {
    ...
    7.4: '#228B22',    # Forest green for neutral zpH 7.4
...
def extract_file_info(filename):
    """Extract pH, fold, and protomer information from filename"""
    # Pattern: FOLD1_pH40_ProtA_internal_interactions.txt
    pattern = r'FOLD(\d+)_pH(\d+)_Prot([AB])_internal_interactions\.txt'
    match = re.search(pattern, filename)
    
    if match:
        fold = int(match.group(1))
        ph_code = int(match.group(2))
        protomer = match.group(3)
        
        # Convert pH code to actual pH value
        ph_map = {40: 4.0, 74: 7.4, 85: 8.5}
        ph_value = ph_map.get(ph_code, ph_code/10.0)
        
        return fold, ph_value, protomer
    
    return None, None, None

def parse_single_file_with_protomer_info(filename, reference_contacts, last_ns_only=600):
    """Parse a single file and return data with protomer information"""
    fold, ph_value, protomer = extract_file_info(filename)
    
    if fold is None or ph_value is None or protomer is None:
    ...
    # Return list of dictionaries with full info
    data_records = []
    for contact_count in contacts_per_frame:
        data_records.append({
            ...
            'fold': fold,
            'protomer': protomer,
            'fold_prot': f'FOLD{fold}_{protomer}',
...
def collect_all_time_series_data(last_ns_only=600):
    """Collect fibril-matching contact data from dimer simulations with protomer tracking"""
    ...
    time_series_files = []
    for file in Path('.').glob('*_internal_interactions.txt'):
    ...
    # Process each file individually (keep protomer info separate)
    for filename in sorted(time_series_files):
        print(f"\n{'='*60}")
        file_data = parse_single_file_with_protomer_info(filename, reference_contacts, last_ns_only)
        data_records.extend(file_data)
    ...
    if not df.empty:
        ...
        print("   Data tracked separately by FOLD and Protomer")
        
        # Summary by pH and fold_prot
        print("\nSummary by pH and FOLD_Protomer:")
        for ph in sorted(df['pH'].unique()):
            print(f"\npH {ph}:")
            ph_data = df[df['pH'] == ph]
            for fold_prot in sorted(ph_data['fold_prot'].unique()):
                fp_data = ph_data[ph_data['fold_prot'] == fold_prot]['contacts']
                mean_recreated = fp_data.mean()
                max_recreated = fp_data.max()
                std_recreated = fp_data.std()
                n_samples = len(fp_data)
                print(f"   {fold_prot}: {mean_recreated:.1f}±{std_recreated:.1f}, Max {max_recreated}, N={n_samples}")
...
def create_violin_plot(df):
    ...
    try:
        ...
        # Add individual points with jitter - colored by FOLD and Protomer
        for i, ph in enumerate(ph_values):
            ...
            # Get fold_prot categories
            fold_prots = df[df['pH'] == ph]['fold_prot'].values
            
            # Define colors and markers for each FOLD_Protomer combination
            fold_prot_styles = {
                'FOLD1_A': {'color': 'darkorange', 'marker': 'o', 'label': 'ANTI A'},
                'FOLD1_B': {'color': 'coral', 'marker': 'o', 'label': 'ANTI B'},
                'FOLD2_A': {'color': 'royalblue', 'marker': 's', 'label': 'PARAL A'},
                'FOLD2_B': {'color': 'lightskyblue', 'marker': 's', 'label': 'PARAL B'}
            }

            for fold_prot in sorted(df['fold_prot'].unique()):
                fold_prot_mask = fold_prots == fold_prot
                if np.any(fold_prot_mask):
                    style = fold_prot_styles.get(fold_prot, {'color': 'gray', 'marker': 'o', 'label': fold_prot})
                    ax.scatter(x_jitter[fold_prot_mask], ph_data[fold_prot_mask], 
                              c=style['color'], 
        ...
        ax.set_ylabel('Fibril Contacts Recreated in Dimer (counts)', fontsize=12, fontweight='bold')
        #ax.set_title('Fibril Contact Recreation - Dimer Internal Contacts (Last 600 ns)', 
        ...
        # Set y-axis limits
        y_min = max(0, df['contacts'].min() - 1)
        y_max_plot = df['contacts'].max() * 1.12
        ax.set_ylim(y_min, y_max_plot)
        
        plt.tight_layout()
        
        # Save the plot
        output_filename = 'Dimer_Fibril_Contacts_Violin_Plot.png'
        try:
            ...
            alt_filename = f'Dimer_Fibril_Contacts_Violin_Plot_{timestamp}.png'
...
def generate_summary_statistics(df):
    ...
    # Create summary table by FOLD_Protomer
    summary_stats = []
    ...
    for ph in sorted(df['pH'].unique()):
        ...
        for fold_prot in sorted(ph_data['fold_prot'].unique()):
            ...
            summary_stats.append({
                'pH': ph,
                'FOLD_Protomer': fold_prot,
                'N_frames': len(fp_data),
                'Mean': fp_data.mean(),
                'Median': fp_data.median(),
                'Std': fp_data.std(),
                'Min': fp_data.min(),
                'Max': fp_data.max(),
                'Q25': fp_data.quantile(0.25),
                'Q75': fp_data.quantile(0.75)
            })
    
    summary_df = pd.DataFrame(summary_stats)
    
    # Save to file
    summary_filename = 'Dimer_Fibril_Contacts_Summary_Statistics.csv'
    ...
    try:
        ...
        import datetime
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        alt_filename = f'Dimer_Fibril_Contacts_Summary_Statistics_{timestamp}.csv'
...
def main():
    """Main analysis pipeline for dimer fibril contact recreation analysis"""
    print("="*60)
    print("Dimer Fibril Contact Recreation Analysis")
    print("="*60)
    print("Analysis approach:")
    print("  • Load fibril inter-chain contacts from 6WQK as reference")
    print("  • Count how many fibril contacts are recreated as INTERNAL")
    print("    contacts within each protomer (ProtA and ProtB)")
    ...
    print("  • Combines data from both ProtA and ProtB")
    ...
    df = collect_all_time_series_data(last_ns_only=50)
    
    if df.empty:
        print("\nNo time series data found!")
        print("Expected files: FOLD#_pH##_Prot[A|B]_internal_interactions.txt")
    ...
    print("  • Dimer_Fibril_Contacts_Violin_Plot.png")
    print("  • Dimer_Fibril_Contacts_Violin_Plot.pdf") 
    print("  • Dimer_Fibril_Contacts_Summary_Statistics.csv")
    print("\nThe plot shows how well internal contacts within each protomer")
    print("(ProtA and ProtB) recreate the inter-chain contact patterns found")
    print("in the 6WQK fibril structure. Data from both protomers are combined.")
    print("Analysis focuses on the last 600 ns (equilibrated) with")
    print("autocorrelation-based systematic subsampling.")
